...
//******************************************************************************
// 色テキスト欄（8桁RGBA 16進）の内容をパレットへ反映
//   ced 20260627: ChooseColor はアルファ非対応のため、各色のテキスト欄を編集可能
//   (EDITTEXT)とし、ここで RRGGBBAA を解析してパレットへ書き戻すことで透明度
//   （末尾2桁）を変更できるようにする。8桁の16進以外は無視して現状維持する。
//******************************************************************************
int MTColorPaletteCfgDlg::_ApplyColorTextFields()
{
	...
	for (i = 0; i < num; i++) {
		...
		//8桁の16進(RRGGBBAA)のみ受理。それ以外は無視。
		if (_tcslen(text) != 8) continue;
		color = DXColorUtil::MakeColorFromHexRGBA(text);
	...
	)
{
	...
	if (!bResult) {
		...
		//新しい色が選択された場合
		//  Windows の ChooseColor はアルファ非対応のため、RGB のみ更新し
		//  アルファ（透明度）は元の色の値を維持する。透明度は色テキスト欄に
		//  8桁 RGBA 16進（末尾2桁がアルファ）を直接入力して変更できる。
		*pNewColor = D3DXCOLOR(
