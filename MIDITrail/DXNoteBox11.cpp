...
struct DXNB11_CONSTANTS {
	...
	XMFLOAT4    opts;     // x = bend whole channel (0/1); yzw unused
...
static const char* DXNB11_SHADER =
	...
	"};\n"
	"struct VSOUT { float4 pos : SV_POSITION; float4 col : COLOR0; float emph : TEXCOORD0; };\n"
	...
	"  return o;\n"
	"}\n"
	"float4 PSMain(VSOUT i) : SV_TARGET { return float4(lerp(i.col.rgb, float3(1,1,1), saturate(i.emph * g_Active.z)), 1.0); }\n";
	...
	)
{
	...
	DXNB11_CONSTANTS c;
	{
		...
		c.opts = XMFLOAT4(m_BendAllNotes ? 1.0f : 0.0f, 0, 0, 0);
