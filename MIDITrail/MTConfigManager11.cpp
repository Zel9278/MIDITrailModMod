...
//******************************************************************************
// 色値ヘルパ（値が 8桁16進 RRGGBBAA なら色とみなす）
//******************************************************************************
static bool _IsColorValue(const char* v)
{
	int n = 0;
	if (v == NULL) return false;
	for (n = 0; v[n] != '\0'; n++) {
		if (n >= 8) return false;               //8桁超は色扱いしない
		if (!isxdigit((unsigned char)v[n])) return false;
	}
	return (n == 8);                            //ちょうど 8桁の16進
}
...
static void _ColorToHex(const float col[4], char* out, size_t outSize)
{
	...
	sprintf_s(out, outSize, "%02X%02X%02X%02X", r, g, b, a);
}
		...
		switch (line.type) {
			...
			case LT_KEYVAL:
				if (sectionOpen) {
					...
					else if (_IsColorValue(line.value)) {
						//色（RRGGBBAA）はカラーピッカーで編集（スウォッチ＋アルファ）
						float col[4];
						_HexToColor(line.value, col);
						ImGui::SetNextItemWidth(220);
						if (ImGui::ColorEdit4(line.key.c_str(), col,
								ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_AlphaPreviewHalf |
								ImGuiColorEditFlags_DisplayHex | ImGuiColorEditFlags_Uint8)) {
							_ColorToHex(col, line.value, sizeof(line.value));
						}
					}
					else {
						ImGui::SetNextItemWidth(220);
