	...
	)
{
	...
	while (readSize < chunkSize) {
		...
		//Report parse progress to the loading screen roughly every 256KB
		if (((readSize & 0x3FFFF) < offset) && (s_LoadProgressFunc != NULL)) {
			s_LoadProgressFunc(readSize, chunkSize, s_LoadProgressUser);
		}
